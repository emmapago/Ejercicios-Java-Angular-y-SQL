...
public class despachos {
	...
	public static void createTable(String db, String name) {
		try {
			...
			String Query = "CREATE TABLE " + name + "" + "(Numero INT PRIMARY KEY, Capacidad INT)";
	...
	public static void insertData(String db, String table_name, int Numero, int Capacidad) {
		try {
			String Querydb = "USE " + db + ";";
			Statement stdb = conector.conexion.createStatement();
			stdb.executeUpdate(Querydb);

			String Query = "INSERT INTO " + table_name + " (Numero, Capacidad) VALUE(" + "\"" + Numero + "\"," + "\""
					+ Capacidad + "\"); ";
			Statement st = conector.conexion.createStatement();
			st.executeUpdate(Query);

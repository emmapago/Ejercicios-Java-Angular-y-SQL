...
public class departamentos {
	...
	public static void createTable(String db, String name) {
		try {
			...
			String Query = "CREATE TABLE " + name + "" + "(Codigo INT PRIMARY KEY, Nombre VARCHAR(100), Presupuesto INT)";
	...
	public static void insertData(String db, String table_name, String name, int Presupuesto) {
		try {
			String Querydb = "USE " + db + ";";
			Statement stdb = conector.conexion.createStatement();
			stdb.executeUpdate(Querydb);

			String Query = "INSERT INTO " + table_name + " (Nombre, Presupuesto) VALUE(" + "\"" + name + "\","+"\""+ Presupuesto +"\");";
			Statement st = conector.conexion.createStatement();
			st.executeUpdate(Query);
